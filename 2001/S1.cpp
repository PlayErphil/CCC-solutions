...
int32_t main() {
	...
	for (auto letter: entry) {
		if (letter == 'C') cur = 0;
		else if (letter == 'D')cur = 1;
		else if (letter == 'H')cur = 2;
		else if (letter == 'S')cur = 3;
		else suits[cur].push_back(letter);
	}
	int sum = 0;
	vector<int> points;
	for (auto suit: suits) {
		int cnt = 0;
		if (suit.size() == 0)cnt += 3;
		else if (suit.size() == 1)cnt += 2;
		else if (suit.size() == 2)cnt += 1;
		for (auto card: suit) {
			if (card == 'A') cnt += 4;
			else if (card == 'K') cnt += 3;
			else if (card == 'Q') cnt += 2;
			else if (card == 'J')cnt += 1;
		}
